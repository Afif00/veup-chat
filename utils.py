...
def process_and_print_query(query, correct_label, response_std, tokens_std, response_simp, tokens_simp):
    ...
    # Print results
    print(f"Query: {query}")
    print(f"  Standard    → Label: {label_std_colored}")
    print(f"  Simplified  → Label: {label_simp_colored}\n")
...
def generate_with_multiple_input(
    ...
    **kwargs,
    ...
    """Generate a chat completion for multiple messages using Amazon Bedrock via ask_claude()."""

    # Prepare the system and formatted messages
    system_prompt, formatted_messages = _prepare_bedrock_messages(messages)

    # Combine all message texts into one string for Claude (since ask_claude expects a single prompt)
    message_text = ""
    if system_prompt:
    ...
    for msg in formatted_messages:
        ...
        # Claude expects text content blocks, join them safely
        text_content = " ".join(
            block.get("text", "") for block in content_blocks if isinstance(block, dict)
        )
        message_text += f"{role.capitalize()}: {text_content}\n\n"
    ...
    # Return the response in the same structure as _invoke_bedrock_chat() would
    return {
        "choices": [
            {
...
class ChatBot:
    ...
    def chat(self, prompt: str, role: str = 'user', return_stats: bool = False) -> Dict[str, str]:
        """
...
class ChatWidget:
    ...
    def __init__(self, generator_function):
        ...
        self.chat_bot = ChatBot(generator_function)  # Initialize chatbot without tracer
    ...
    def process_bot_response(self, user_message):
        ...
        try:
            ...
            self.display_error_box(exc, tb)

    def display_error_box(self, exc: Exception, traceback_text: str | None = None):
        """Render an error box into the chat transcript."""
        err_html = (
            "<div style='background-color:#fdecea;border:1px solid #f5c6cb;"
            "padding:10px;margin:5px 0;border-radius:8px;max-width:90%;color:black !important;'>"
            "<strong>Assistant (error):</strong>"
            f"<div style='margin-top:6px;white-space:pre-wrap;color:#721c24;'>{markdown.markdown(str(exc))}</div>"
        )
        if traceback_text:
            safe_tb = markdown.markdown(f"```\n{traceback_text}\n```")
            err_html += (
                "<details style='margin-top:8px;'><summary>Show traceback</summary>"
                f"<div style='margin-top:6px;'>{safe_tb}</div></details>"
            )
        err_html += "</div>"
